    ...
    <div>
        <table id="grid" class="table table-condensed table-hover">
            ...
            <tr v-for="(entry,index) in filteredData" :key="index">
                ...
                <td v-if="!grid.excludeIndex">{{index + 1}}</td>
                ...
                <td v-for="key in grid.columns" :class="key.class" :style="key.style">
                    ...
                    <div v-if="isIncludeEdit(key)">
                        ...
                        <!-- checkbox -->
                        <input v-if="key.itype == 'selector'" type="checkbox" />

                        <!-- plain text -->
                        <input type="text" class="form-control" :class="key.class"
                               v-if="key.itype == 'text'"
                    ...
                    <div v-if="key.name=='action'" class="btn-group">
                        ...
                        <ul class="dropdown-menu" >
                            <li v-for="action in actionButtons"><a href="#" @click='actionTrigger(action,entry["id"],index)'>{{action.name}}</a></li>
                        </ul>
                    </div>
                    <div v-if="key.name=='$markDelete'" class='text-center'>
                        <button class="btn btn-danger btn-xs" type="button" @click='actionTrigger("delete",entry["id"],index)'><i class="fa fa-close"></i></button>
                    </div>
                </td>
            </tr>
            </tbody>
        ...
        <div v-if="paginate">
            <pagination :param="data" @click="onPaginateClick"></pagination>
        </div>
    </div>
</template>
