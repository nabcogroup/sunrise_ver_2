    ...
    public function execute()
    {
        ...
        $records = $this->createDb('contracts')
                ...
                ->select('contracts.id','villas.villa_no',
                    ...
                    $this->sqlClearedPayment('contract_bill_id','gross_sale'))
                ->where('villas.location',$location)
                ->where('contracts.status','active')
                ->distinct()
                ->orderBy('villa_no');

        $rows = $this->arrayGroupBy($records->get(),function($row) {
        ...
        $title = "Active Contracts - "  . Selection::getValue("villa_location",$location);
